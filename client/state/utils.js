 ...
 * @param  {Object}   definedHandlers Object mapping action types to state
 *                                    action handlers
 * @param  {?Object}  schema          JSON schema object for deserialization
 *                                    validation
 * @return {Function}                 Reducer function
 */
export function createReducer( initialState = null, definedHandlers = {}, schema = null ) {
	...
	if ( definedHandlers[ SERIALIZE ] ) {
		...
		handlers[ SERIALIZE ] = ( state, action ) => {
			...
			const serialized = definedHandlers[ SERIALIZE ]( state, action );
			lastState = state;
